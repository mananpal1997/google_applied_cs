...
public class TreeNode {
    ...
    public void insert(int valueToInsert) {
        if(value == valueToInsert) return;
        if(value < valueToInsert) {
            if(right == null) {
                TreeNode new_node = new TreeNode(valueToInsert);
                new_node.left = null;
                new_node.right = null;
                right = new_node;
            } else {
...
//        transformAVL(bal);
    ...
    public void transformAVL(int balance_factor) {
        if(balance_factor < -1) leftRotate();
        else if(balance_factor > 1) rightRotate();
    }
...
//
//        height = treeHeight();
        TreeNode temp1 = right;
        TreeNode temp2 = temp1.left;

        temp1.left = this;
        right = temp2;

        temp1.height = max(treeHeight(temp1.left), treeHeight(temp1.right)) + 1;
        height = treeHeight(this);
